...
def get_cot_latent_dataset(
    ...
    Args:
        ...
        override_uniform_prob: If not None, override configs.uniform_prob.
            Set to 0.0 during consolidation epochs to disable random stage
            sampling and force all samples to use the scheduled stage.
    """
    n_additional_tokens = 0 if no_special_marker else 2  # start + end markers
    effective_uniform_prob = (
        override_uniform_prob if override_uniform_prob is not None
        else configs.uniform_prob
    )
    ...
    def process_dataset(sample):
        ...
        if (
            ...
            and rng.random() < effective_uniform_prob
        ):
